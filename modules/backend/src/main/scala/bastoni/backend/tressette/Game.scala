...
object Game:
  ...
  private[tressette] val playMatchStep: (MatchState, Command | Event) => (MatchState, List[Event | Command | DelayedCommand]) = {
    ...
    case (MatchState.DealRound(player :: Nil, done, 0, deck), Continue) =>
      deck.dealOrDie { (card, tail) =>
        ...
        MatchState.PlayRound(players, Nil, tail) -> List(CardDealt(player.id, card), ActionRequest(players.head.id, Action.PlayCard))
      }
    ...
    case (MatchState.DrawRound(player :: todo, done, deck), Continue) =>
      deck.dealOrDie { (card, tail) => MatchState.DrawRound(todo, done :+ player.draw(card), tail) -> List(CardDealt(player.id, card), Continue.shortly) }
  ...
  private[tressette] val playGameStep: (GameState, Command | Event) => (GameState, List[Event | Command | DelayedCommand]) = {

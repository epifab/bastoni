...
object PlayerState:
  ...
  case class ActingPlayer(override val player: MatchPlayer, action: Action, timeout: Option[Timeout])
  ...
  given Encoder[PlayerState] = Encoder.instance {
    ...
    case obj: EndOfMatchPlayer =>
      deriveEncoder[EndOfMatchPlayer].mapJsonObject(_.add("state", "EndOfMatch".asJson))(obj)
  }

  given Decoder[PlayerState] = Decoder.instance { cursor =>
    cursor.downField("state").as[String].flatMap {

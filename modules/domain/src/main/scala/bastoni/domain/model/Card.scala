...
case class VisibleCard(rank: Rank, suit: Suit, ref: CardId) extends CardInstance with Card:
  ...
  override def hidden: HiddenCard = HiddenCard(ref, Some(simple))
...
case class HiddenCard(ref: CardId, card: Option[Card] = None) extends CardInstance:
  override def toOption: Option[VisibleCard] = card.map(c => VisibleCard(c.rank, c.suit, ref))
  override def hidden: HiddenCard = this

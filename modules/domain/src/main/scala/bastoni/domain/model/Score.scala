...
object GameScore:
  given Encoder[GameScore] = Encoder.instance(score => Json.obj(
  ...
  given Decoder[GameScore] = Decoder.instance(cursor =>
    for {
      ...
      details <- cursor.downField("details").focus.toRight(DecodingFailure("Details not found", cursor.downField("details").history))

...
object ScopaGameScoreCalculator:
  ...
  def apply(teams: List[List[Player]]): List[ScopaGameScore] =
    ...
    val denari: Option[(List[Player], ScopaGameScoreItem)] =
      teamWithCards.view
        .mapValues(cards => ScopaGameScoreItem.Denari(cards.count(_.suit == Denari)))
        .toList
        .bestBy(_._2.count)
    ...
    val setteBello: Option[(List[Player], ScopaGameScoreItem)] =
      teamWithCards
        .collectFirst {
          case (team, cards) if cards.exists(c => c.rank == Sette && c.suit == Denari) =>

...
abstract class GenericGameLogic extends GameLogic[MatchState]:
  ...
  override val play: (MatchState, StateMachineInput) => (MatchState, List[StateMachineOutput]) = {
    ...
    case (inProgress: MatchState.InProgress, message) =>
      val gameState: GameState =
        inProgress.gameState.as[GameState].getOrElse(throw new IllegalStateException("Game state corrupted"))
      ...
      statusFor(updatedGameState) match
        ...
        case GameStatus.Completed(playersUpdate) =>
          val updatedPlayers = inProgress.players.map(possiblyOutdated =>
            playersUpdate.find(_.is(possiblyOutdated)).getOrElse(possiblyOutdated)
          )
          ...
          val winners: Option[MatchScore] = inProgress.matchType match
            case MatchType.FixedRounds(remainingGames) =>
              sortedTeams match
                case winningTeam :: secondTeam :: _ if winningTeam.points > remainingGames + secondTeam.points =>
                  Some(winningTeam)
                case _ => None
          ...
          val nextState = winners.fold(nextGame)(score =>
            MatchState.GameOver(Event.MatchCompleted(score.playerIds), MatchState.Terminated)
          )
          val playerIds = updatedPlayers.map(_.id)
          WaitingForPlayers(playerIds.toSet, nextState) -> (events ++ playerIds.map(id =>
            Command.Act(id, Action.Confirm, timeout = None)
          ))

        case GameStatus.Aborted(reason) =>
          MatchState.GameOver(Event.MatchAborted(reason), MatchState.Terminated) -> (events :+ Event.GameAborted(
            reason
          ) :+ Continue.afterGameOver)
  ...
  @tailrec
  private def playSteps(
      state: MatchState,
      input: List[StateMachineInput],
      previousEvents: List[StateMachineOutput] = Nil
  ): (MatchState, List[StateMachineOutput]) =
    input match

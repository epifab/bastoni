...
object RegressionSpecContent:
  given Codec[StateMachineInput] = Codec.from(StateMachineCodecs.inputDecoder, StateMachineCodecs.inputEncoder)
  given Codec[StateMachineOutput] = Codec.from(StateMachineCodecs.outputDecoder, StateMachineCodecs.outputEncoder)
    ...
    for {
      ...
      _ <- (for {
        ...
        initialTable = TableServerView(
          seats = users.map(u => Seat(Some(WaitingPlayer(MatchPlayer(u, 0))), Nil, Nil, Nil)),
          deck = Nil,
          board = Nil,
          active = false
        )

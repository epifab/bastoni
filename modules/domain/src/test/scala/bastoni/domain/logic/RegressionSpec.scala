...
class RegressionSpec extends AsyncIOFreeSpec:
  ...
  yield s"$gameType with $numberOfPlayers players" in {
    for
      json <- IO(
        Source.fromResource(s"${gameType.toString.toLowerCase}-$numberOfPlayers-players-spec.json").getLines().mkString
      )
